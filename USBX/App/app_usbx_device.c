...
UINT MX_USBX_Device_Init(VOID *memory_ptr) {
  ...
  tx_status = tx_byte_allocate(byte_pool, (VOID **)&pointer, USBX_MEMORY_SIZE,
                               TX_NO_WAIT);
  ...
  tx_status = tx_thread_create(
      &ux_app_thread, "main_usbx_app_thread_entry", usbx_app_thread_entry, 0,
      pointer, USBX_APP_STACK_SIZE, 20, 20, TX_NO_TIME_SLICE, TX_AUTO_START);
  ...
  tx_status = tx_thread_create(
      &ux_cdc_read_thread, "cdc_acm_read_usbx_app_thread_entry",
      usbx_cdc_acm_read_thread_entry, 1, pointer, USBX_APP_STACK_SIZE, 20, 20,
  ...
  /* Create the usbx_cdc_acm_thread_entry thread. */
  tx_status = tx_thread_create(
      &ux_cdc_write_thread, "cdc_acm_write_usbx_app_thread_entry",
      usbx_cdc_acm_write_thread_entry, 1, pointer, USBX_APP_STACK_SIZE, 20, 20,
      TX_NO_TIME_SLICE, TX_AUTO_START);
