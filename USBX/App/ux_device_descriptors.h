...
typedef struct {
  ...
  uint8_t bReserved;
    ...
    (USBD_CMPSIT_ACTIVATE_RNDIS == 1) || (USBD_CDC_ECM_CLASS_ACTIVATED == 1)
typedef struct {
...
#endif /* (USBD_CDC_ACM_CLASS_ACTIVATED == 1) || (USBD_CMPSIT_ACTIVATE_RNDIS \
          == 1)  || (USBD_CDC_ECM_CLASS_ACTIVATED == 1)*/
...
#define __USBD_FRAMEWORK_SET_IF(ifnum, alt, eps, class, subclass, protocol, \
                                istring)                                    \
  do {                                                                      \
